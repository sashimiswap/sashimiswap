...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should set correct state variables', async () => {
        this.chef = await MasterChef.new(this.sashimi.address, dev, '1000', '0', '1000', { from: alice });
        await this.sashimi.transferOwnership(this.chef.address, { from: alice });
        const sashimi = await this.chef.sashimi();
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await MasterChef.new(this.sashimi.address, dev, '100', '100', '1000', { from: alice });
        ...
        it('should not distribute SASHIMIs if no one deposit', async () => {
            ...
            this.chef = await MasterChef.new(this.sashimi.address, dev, '100', '200', '1000', { from: alice });
            await this.sashimi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
        ...
        it('should distribute SASHIMIs properly for each staker', async () => {
            ...
            this.chef = await MasterChef.new(this.sashimi.address, dev, '100', '300', '1000', { from: alice });
            await this.sashimi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            ...
            assert.equal((await this.sashimi.balanceOf(this.chef.address)).valueOf(), '4334');
            assert.equal((await this.sashimi.balanceOf(dev)).valueOf(), '1000');
            ...
            assert.equal((await this.sashimi.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.sashimi.balanceOf(this.chef.address)).valueOf(), '8144');
        ...
        it('should give proper SASHIMIs allocation to each pool', async () => {
            ...
            this.chef = await MasterChef.new(this.sashimi.address, dev, '100', '400', '1000', { from: alice });
            await this.sashimi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            ...
            assert.equal((await this.chef.pendingSashimi(0, alice)).valueOf(), '10000');
            // Bob deposits 10 LP2s at block 425
            await time.advanceBlockTo('424');
            await this.chef.deposit(1, '5', { from: bob });
            // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
            assert.equal((await this.chef.pendingSashimi(0, alice)).valueOf(), '11666');
            await time.advanceBlockTo('430');
            // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
            assert.equal((await this.chef.pendingSashimi(0, alice)).valueOf(), '13333');
            assert.equal((await this.chef.pendingSashimi(1, bob)).valueOf(), '3333');
        });

        it('should stop giving bonus SASHIMIs after the bonus period ends', async () => {
            // 100 per block farming rate starting at block 500 with bonus until block 600
            this.chef = await MasterChef.new(this.sashimi.address, dev, '100', '500', '600', { from: alice });
            await this.sashimi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            ...
            assert.equal((await this.chef.pendingSashimi(0, alice)).valueOf(), '10500');
            // At block 606, Alice withdraws all pending rewards and should get 10600.
            await this.chef.deposit(0, '0', { from: alice });
            assert.equal((await this.chef.pendingSashimi(0, alice)).valueOf(), '0');
